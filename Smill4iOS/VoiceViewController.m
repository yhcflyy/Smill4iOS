...
-(void)refreshData{
    __weak VoiceViewController *weakSelf = self;
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    NSDictionary *parameters = @{@"a": @"newlist",@"type":@"31",@"c":@"voice",@"page":@"1",@"per":@"15"};
    [manager GET:API_URL parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        self.curPage=1;
        VoiceModel *picModel=[[VoiceModel alloc] initWithDictionary:responseObject error:nil];
...
- (PSCollectionViewCell *)collectionView:(PSCollectionView *)collectionView cellForRowAtIndex:(NSInteger)index {
    ...
    VoiceInfo *infoModel=self.modelsArray[index];
    VoiceViewCell *cell=(VoiceViewCell*)[self.collectionView dequeueReusableViewForClass:[VoiceViewCell class]];
    if(cell == nil){
        cell= [[VoiceViewCell alloc] initWithFrame:CGRectZero];
    }
